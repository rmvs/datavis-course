  ...
  main.variable(observer("buildvis")).define("buildvis", ["magnitudeChart","magDimension","magDimensionCount","d3","updateMarkers","depthChart","depthDimension","depthDimensionCount","timeChart","hourDimension","hourDimensionCount","dataset","dataTable","dateDimension","dc"], function(magnitudeChart,magDimension,magDimensionCount,d3,updateMarkers,depthChart,depthDimension,depthDimensionCount,timeChart,hourDimension,hourDimensionCount,dataset,dataTable,dateDimension,dc)
{
  ...
  main.variable(observer("visibleMarkers")).define("visibleMarkers", function()
{
  let intern = []
...
map.on("moveend",updateFilters)
  ...
  main.variable(observer("updateFilters")).define("updateFilters", ["layerList","visibleMarkers","idDimension","dataset","crossfilter","magnitudeChart","dc"], function(layerList,visibleMarkers,idDimension,dataset,crossfilter,magnitudeChart,dc){return(
  ...
  if(layerList != null && layerList.length > 0){      
    layerList[0].eachLayer(function(layer) {
        if( e.target.getBounds().contains(layer.getLatLng()) ){
          ...
          mymarkers.push(layer);
        }
   });     
  }
  
  var temp = idDimension.filterFunction(function(d){
      return mymarkers.indexOf(d) > -1;
  }).group().all();
  
  if(temp != null && temp.length > 0){
   var new_data = [];
      temp.forEach(function(v){
          new_data.push(dataset.find(k => k.publicid == v.key));          
      })
    var magDim = crossfilter(new_data).dimension(s => s.magnitude);
    var magCount = magDim.group();
    magnitudeChart.dimension(magDim);
    console.log(magCount);
    magnitudeChart.group(magCount);
    dc.redrawAll();    
  } 
}
)});
...
idDimension.group()
  ...
  main.variable(observer()).define(["idGrouping"], function(idGrouping){return(
idGrouping.all()
)});
  main.variable(observer()).define(["idGrouping"], function(idGrouping){return(
idGrouping.all()
