...
html`<code>css</code> <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.0/css/bootstrap.min.css" integrity="sha384-PDle/QlgIONtM1aqA2Qemk5gPOE7wFq8+Em+G/hmo5Iq0CCmYZLv3fVRDJ4MMwEA" crossorigin="anonymous">`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","dateDimension","d3","magnDimension","magnGroup","depthDimension","depthGroup","dataset","terremotoDim","terremotoGroup"], function(md,container,dc,dateDimension,d3,magnDimension,magnGroup,depthDimension,depthGroup,dataset,terremotoDim,terremotoGroup)
{
  let view = md`${container('dc-table-graph','Gráfico')}`;
  ...
  barChart.width(480)
         ...
         .gap(56)
         .elasticY(true)
         .dimension(magnDimension)
         .group(magnGroup,"Magnitude");
  ...
  let terremotosChart = dc.lineChart(view.querySelector("#time-chart"))
      terremotosChart.width(960)
                     .height(150)
                     .x(d3.scaleTime().domain(d3.extent(dataset, d => d.dtg)))
                     .dimension(terremotoDim)
                     .group(terremotoGroup);
      terremotosChart.render();
...
d3.csv('https://gist.githubusercontent.com/emanueles/65a308ffa630689c11a031252998ef8d/raw/a004c770786229d54264406118ae21ba7e4c51a8/earthquakes.csv').then(function(data){
    var parser = d3.utcParse("%Y-%m-%dT%H:%M:%S");
    var magnFormatter = d3.format(".1f");
    var depthFormatter = d3.format("d");
    data.forEach(function(d,i){
        d.dtg = parser(d.origintime.substr(0,19));
        d.magnitude = magnFormatter(d.magnitude);
...
facts.dimension(d => d.dtg)
  ...
  main.variable(observer("magnDimension")).define("magnDimension", ["facts"], function(facts){return(
...
facts.dimension(d => d.depth)
  ...
  main.variable(observer("magnGroup")).define("magnGroup", ["magnDimension"], function(magnDimension){return(
magnDimension.group()
)});
  main.variable(observer("depthGroup")).define("depthGroup", ["depthDimension"], function(depthDimension){return(
depthDimension.group()
)});
  main.variable(observer("terremotoDim")).define("terremotoDim", ["facts","d3"], function(facts,d3){return(
...
<main role="main" class="container">
    ...
    <div class='row'>
        <div id='magnitude-chart' class="col-12">
          <h5> Number of Events by Magnitude </h5>
        </div>        
    </div>
    <div class="row">
      <div id='depth-chart' class="col-12">
