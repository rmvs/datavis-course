  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","topojson","us","colorScale","rateById","showTooltip","hideTooltip"], function(d3,DOM,topojson,us,colorScale,rateById,showTooltip,hideTooltip)
{
    ...
    .enter().append("path")
      ...
      .on('mouseover', function(d){
          d3.select(this)
            ...
            .attr('stroke-width',3)
            .attr('stroke','#FFF5B1')
            //.append('title').text(d => `Taxa de desemprego: ${rateById.get(d.id)}%`);
  ...
  main.variable(observer("condados")).define("condados", ["d3"], function(d3){return(
...
d3.scaleQuantize()
  ...
  main.variable(observer("showTooltip")).define("showTooltip", ["d3","rateById","nomesMap"], function(d3,rateById,nomesMap){return(
...
.text(rateById.get(county_id))
 ...
 d3.select("#nome").text(nomesMap.get(county_id).name);
  ...
  main.variable(observer("tooltip")).define("tooltip", ["d3"], function(d3)
{
  ...
  let node = d3.select("body")
               ...
               .attr("class","hidden")
               .append("p")
               .html("<span id='nome'></span><br/>Taxa de desemprego: <span id='taxa'></span>%")
  return node;
}
);
...
d3.json("https://gist.githubusercontent.com/emanueles/ee2997bc6237f3dcab952ae8becaf6bf/raw/afdb12d4ef89bb239998b295c94d960e8d89c19e/us.json")
  ...
  main.variable(observer()).define(["nomesMap"], function(nomesMap){return(
nomesMap
)});
  main.variable(observer("nomesMap")).define("nomesMap", ["d3","condados"], function(d3,condados){return(
d3.map(condados,function(q){ 
  return q.id
})
)});
